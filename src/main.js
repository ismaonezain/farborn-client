...
function playerAttack() {
  ...
  // [DEFERRED] Server-authoritative combat — format sync needed
  // Server tick runs in background, reconciles gold/exp on next sync
  queueEvent(EVENT_TYPES.COMBAT_TICK, { zone: state.zone, ts: Date.now() });
}
...
function mobAttack() {
  ...
  // [DEFERRED] Server combat reconciliation — format sync needed
  queueEvent(EVENT_TYPES.COMBAT_TICK, { zone: state.zone, ts: Date.now() });
}
